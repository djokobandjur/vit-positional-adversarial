"""
CIFAR-100 Training (v2.0)
=========================
Trains 12 ViT models (4 PE types × 3 seeds) on CIFAR-100.

NOTE (v2.0): The attack code in this file (PHASE 2 of __main__, and the
functions in the ADVERSARIAL ATTACKS section below) is DEPRECATED. It
contains the multi-block bug from the withdrawn v1.0 paper: the
perturbation δ targeted only model.blocks[0] instead of all 12 transformer
blocks, attacking only 1/12 of the RoPE/ALiBi mechanism.

The deprecated attack code is retained for transparency and historical
reference, but it is NOT executed — __main__ exits after PHASE 1
(training). For corrected v2.0 attacks on CIFAR-100, use:

    python full_reanalysis.py --dataset cifar \\
        --models_dir <path> --output_path <path>

See README and CHANGELOG for the full v2.0 attack pipeline.

Usage in Colab (training only):
  1. Mount Drive
...
# ========================================================================================
# CIFAR-100 CONFIG (differs from ImageNet-100 in img_size and patch_size parameters only )
# ========================================================================================
CIFAR_CONFIG = {
    'img_size': 32,
...
# =============================================================================
# DEPRECATED ATTACK CODE — DO NOT USE
# =============================================================================
# The attack functions below (get_pe_variance_weights, fgsm_pe_attack,
# pgd_pe_attack, vta_attack) were used in the v1.0 (withdrawn) version of
# this work. They contain the multi-block PGD bug: the perturbation δ
# targeted only model.blocks[0].attn.rope.cos_cached (and analogous ALiBi
# slopes), attacking only 1/12 of the RoPE/ALiBi mechanism instead of the
# intended shared-δ across all 12 transformer blocks.
#
# For correct v2.0 attack reproduction (multi-block corrected), use:
#     python full_reanalysis.py --dataset cifar \
#         --models_dir <path> --output_path <path>
#
# This code is retained for transparency and reproducibility of the
# withdrawn v1.0 CIFAR-100 results. PHASE 2 of __main__ below does NOT
...
if __name__ == '__main__':
    ...
    # ========== PHASE 2: ATTACKS (DEPRECATED — NOT EXECUTED) ==========
    # The attack loop below contains the v1.0 multi-block bug. It is
    # retained for transparency and historical reference but is NOT
    # executed. For correct v2.0 attacks on CIFAR-100, use:
    #     python full_reanalysis.py --dataset cifar \
    #         --models_dir <path> --output_path <path>
    print("\n" + "="*60)
    print("  PHASE 1 complete (training only).")
    print("")
    print("  Attacks are NOT run by this script in v2.0.")
    print("  For corrected v2.0 attacks, run:")
    print("    python full_reanalysis.py --dataset cifar \\")
    print("        --models_dir <path> --output_path <path>")
    print("")
    print("  See README and CHANGELOG for full v2.0 pipeline.")
    print("="*60)
    sys.exit(0)

    # --- DEPRECATED v1.0 ATTACK CODE BELOW (NOT EXECUTED) ---
    # ========== PHASE 2: ATTACKS ==========
    print("\n" + "="*60)
